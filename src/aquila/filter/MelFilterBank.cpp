...
MelFilterBank::MelFilterBank(const double sample_rate, const std::size_t fft_size,
    ...
    for (std::size_t f = 0; f < num_filters; ++f) {
        ...
        const auto bin_min = static_cast<std::size_t>(
            freq_min * static_cast<double>(fft_size) / sample_rate);
        auto bin_max = static_cast<std::size_t>(
            freq_max * static_cast<double>(fft_size) / sample_rate);
        bin_max = std::min(bin_max, half_n - 1);

        if (bin_max <= bin_min) continue;

        // Only store non-zero weights (sparse).
        for (std::size_t k = bin_min; k <= bin_max; ++k) {
            const double freq_k = static_cast<double>(k) * sample_rate /
