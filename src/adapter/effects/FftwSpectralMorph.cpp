...
SpectralMorph::SpectralMorph(std::shared_ptr<port::IFft> fft)
    : fft_(std::move(fft)) {}

std::vector<double> SpectralMorph::apply(
    ...
    for (std::size_t k = 0; k < half; ++k) {
        ...
        morphed[k] = {mag_morph * std::cos(phase_morph),
                      mag_morph * std::sin(phase_morph)};
    }
