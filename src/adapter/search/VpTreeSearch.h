 ...
 *
 * For each target fingerprint, queries the nearest-neighbour index via
 * `brain.kNearest(target_fp, k)` to obtain a small candidate set, then
 * scores each candidate with `SearchUtils::fullScore()` (which applies the
 * same per-feature weights / n_ratio / usage penalty logic as ClosestSearch)
