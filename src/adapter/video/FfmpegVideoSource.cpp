    ...
    CtxPtr acquire(const std::string& path) {
        ...
        for (std::size_t i = 0; i < ctx->fmt_ctx.streamsCount(); ++i) {
            ...
            if (st.isVideo()) {
                ...
                const auto dur = ctx->fmt_ctx.duration();
                if (dur)
...
std::unique_ptr<Sound> FfmpegVideoSource::loadAudio(const std::string& path) {
    ...
    for (std::size_t i = 0; i < ictx.streamsCount(); ++i) {
        if (ictx.stream(i).isAudio()) { audio_stream = static_cast<ssize_t>(i); break; }
    }
    if (audio_stream < 0) return nullptr;
    ...
    av::AudioResampler resampler(
        AV_CH_LAYOUT_STEREO,       pimpl_->target_sr, av::SampleFormat("dblp"),
    ...
    auto collect = [&](const av::AudioSamples& out) {
        ...
        for (int ch = 0; ch < kOutChannels; ++ch) {
            const auto* src = reinterpret_cast<const double*>(out.data(static_cast<std::size_t>(ch)));
            channels[static_cast<std::size_t>(ch)].insert(
                channels[static_cast<std::size_t>(ch)].end(),
                src, src + out.samplesCount());
        }
    };
    ...
    while (true) {
        ...
        if (ec || !pkt) break;
        if (pkt.streamIndex() != static_cast<size_t>(audio_stream)) continue;
    ...
    if (channels.empty() || channels[0].empty()) return nullptr;
...
bool FfmpegVideoSource::getInfo(const std::string& path,
                                int& width, int& height,
                                double& fps, double& duration_seconds) {
    auto ctx = pimpl_->acquire(path);
    if (!ctx || ctx->vid_stream < 0) return false;
    width            = ctx->width;
    height           = ctx->height;
    fps              = ctx->fps;
...
std::optional<VideoFrame> toRgb(const av::VideoFrame& frame, int out_w, int out_h) {
    ...
    for (int row = 0; row < out_h; ++row) {
        std::memcpy(vf.pixels.data() + static_cast<std::size_t>(row) * row_bytes,
                    rgb.data(0) + row * rgb.raw()->linesize[0],
                    row_bytes);
    }
    return vf;
}
...
std::optional<VideoFrame> FfmpegVideoSource::readFrame(
    const std::string& path, const double time_seconds) {

    auto ctx = pimpl_->acquire(path);
    if (!ctx || ctx->vid_stream < 0) return std::nullopt;

    std::error_code ec;
    const av::Timestamp ts{
        static_cast<int64_t>(time_seconds * av::TimeBase),
    ...
    while (true) {
        ...
        if (ec || !pkt) break;
        if (pkt.streamIndex() != static_cast<std::size_t>(ctx->vid_stream)) continue;

        av::VideoFrame frame = ctx->vid_dec.decode(pkt, ec);
        if (ec || !frame) { ec.clear(); continue; }

        if (frame.pts().seconds() + frame_dur <= time_seconds) continue;
...
std::vector<VideoFrame> FfmpegVideoSource::readSegment(
    const std::string& path, const double start_seconds, const double end_seconds) {

    std::vector<VideoFrame> result;
    if (end_seconds <= start_seconds) return result;

    auto ctx = pimpl_->acquire(path);
    if (!ctx || ctx->vid_stream < 0) return result;
    ...
    const bool need_seek = !buffer_usable &&
                           ((ctx->current_pts < 0.0)
                            || (start_seconds < ctx->current_pts - frame_dur * 0.5)
    ...
    while (true) {
        ...
        if (pts >= end_seconds) {
            auto vf = toRgb(frame, ctx->width, ctx->height);
            if (vf) {
