...
std::vector<double> StreamProcessor::applyEffects(
    ...
    if (spectral_morph_ && params_.spectral_morph > 0.0 && !prev_block_.empty()) {
        out = spectral_morph_->apply(prev_block_, out, params_.spectral_morph);
    }
    prev_block_ = out;

    // Stutter.
    effects::applyStutter(out, params_.stutter_chance, params_.stutter_count);

    // Envelope shaping.
    effects::applyEnvelope(out, params_.envelope_shape, params_.envelope_amount);
...
bool StreamProcessor::stream(Brain& brain, const Sound& target) {
    ...
    while (running_) {
        for (std::size_t pos = 0; pos + bs <= ch0.size() && running_; pos += step) {
            ...
            const auto& match = brain.findBestMatch(fp, params_);
            ...
            for (std::size_t ch = 0; ch < num_channels; ++ch) {
                ...
                auto src = applyEffects(*src_ptr, bs);
...
void StreamProcessor::streamInfinite(Brain& brain, const int sample_rate,
    ...
    brain.jiggle();
    std::vector<double> search_fp = brain.blocks()[brain.currentBlockIndex()].fingerprint;

    // Compute the typical scale of fingerprint values so drift is meaningful.
    // Use the RMS of the seed fingerprint as reference.
    double fp_scale = 0.0;
    ...
    if (fp_scale < 1e-6) fp_scale = 1.0;  // Fallback for silent blocks.

    // In infinite mode, override usage params to force variety if they're
    // at defaults (no-op). This prevents freezing on popular blocks.
    SearchParams inf_params = params_;
    inf_params.usage_weight  = std::max(params_.usage_weight, 0.1);
    const double inf_usage_falloff = std::min(params_.usage_falloff, 0.995);
    ...
    while (running_) {
        ...
        if (stuck_count > 3) {
            brain.jiggle();
            search_fp = brain.blocks()[brain.currentBlockIndex()].fingerprint;
            stuck_count = 0;
        }

        // Evolve search fingerprint: blend toward match + additive drift.
        // The drift magnitude is proportional to the fingerprint's overall
        // scale, not individual coefficient values, so all dimensions move.
        const auto& match_fp = match.fingerprint;
        search_fp.resize(match_fp.size());

        // Slowly oscillating drift amount to vary exploration intensity.
        ...
        for (std::size_t ch = 0; ch < num_ch; ++ch) {
            ...
            out_channels[ch] = applyEffects(*src_ptr, bs);
        }
